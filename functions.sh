...
get_version()
{
	...
	if [[ "$VERSION" == *"gpdb"* ]]; then
		...
		if [ "$zstd_test" -eq "1" ]; then
			SMALL_STORAGE="appendonly=true, orientation=column, blocksize=8192"
			MEDIUM_STORAGE="appendonly=true, orientation=column, compresstype=zstd, blocksize=8192"
			LARGE_STORAGE="appendonly=true, orientation=column, compresstype=zstd, blocksize=8192"
		else
